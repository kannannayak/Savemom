...
if ($p_id) {
    $query = "SELECT * FROM patient_scan_files WHERE p_id = :p_id AND file_path LIKE '%prescription%'";
    ...
    if ($results) {
        ...
        $response = ['status' => false, 'message' => 'No prescription files found for the given p_id' . ($r_id !== null ? ' and r_id.' : '.')];
    }
} else {
